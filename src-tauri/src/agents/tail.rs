        ...
        if len > offset {
            ...
            match read_chunk(file, offset) {
                Ok((bytes, text)) => {
                    ...
                    let mut reset = was_first;
                    while let Some(pos) = rem.find('\n') {
                        let line: String = rem.drain(..=pos).collect();
                        let line = line.trim();
                        ...
                        if batch.len() >= BATCH {
                            emit(&app, tail_id, reset, !was_first, std::mem::take(&mut batch));
                            reset = false;
                        }
                    }
                    if !batch.is_empty() || reset {
                        emit(&app, tail_id, reset, !was_first, batch);
                    }
                    if was_first {
            ...
            emit(&app, tail_id, true, true, Vec::new());
            first = false;
        }
...
fn read_chunk(file: &str, offset: u64) -> std::io::Result<(u64, String)> {
    ...
    Ok((n, String::from_utf8_lossy(&buf).into_owned()))
}
...
fn tool_event(block: &serde_json::Value, at: i64, side: bool, cwd: &str) -> Option<FeedEvent> {
    ...
    match name {
        ...
        _ => {
            ...
            let pretty = name.strip_prefix("mcp__").unwrap_or(name).replace("__", " · ");
...
mod tests {
    ...
    #[test]
    fn edit_vira_evento_com_diff_e_caminho_relativo() {
        let evs = parse_all(&[r#"{"type":"assistant","timestamp":"2026-08-13T21:09:41.434Z","cwd":"C:\\proj","message":{"id":"m1","model":"claude-opus-5","content":[{"type":"tool_use","id":"t1","name":"Edit","input":{"file_path":"C:\\proj\\src\\a.ts","old_string":"a\nb","new_string":"a\nb\nc"}}],"usage":{"input_tokens":2,"output_tokens":10}}}"#]);
    ...
    #[test]
    fn usage_deduplica_por_mensagem() {
        let line = |id: &str| {
            format!(
    ...
    #[test]
    fn resultado_extrai_task_id_e_erro() {
        let evs = parse_all(&[r#"{"type":"user","message":{"content":[{"type":"tool_result","tool_use_id":"t9","content":"Task #7 created successfully: x","is_error":false}]}}"#]);
    ...
    #[test]
    fn wrapper_de_comando_nao_e_prompt_mas_notificacao_passa() {
        let evs = parse_all(&[
    ...
    #[test]
    fn subagente_carrega_tipo_e_descricao() {
        let evs = parse_all(&[r#"{"type":"assistant","message":{"id":"m3","content":[{"type":"tool_use","id":"t2","name":"Agent","input":{"subagent_type":"Explore","description":"mapear o repo","prompt":"..."}}]}}"#]);
        let tool = &evs[0];
