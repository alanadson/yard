...
pub fn watch<R: Runtime>(
    ...
    Ok(WatchHandle { _watcher: watcher, root })
}
...
fn ingest(
    batch: &mut HashMap<PathBuf, PathState>,
    dropped: &mut u32,
    root: &Path,
    event: Event,
) {
...
mod tests {
    ...
    #[test]
    fn filtra_diretorios_ruidosos_e_lixo() {
        ...
        assert!(!ignored(root, Path::new("C:\\proj\\.vscode\\settings.json")));
    }

    #[test]
    fn caminho_relativo_com_barras_normais() {
        let root = Path::new("C:\\proj");
        assert_eq!(relative(root, Path::new("C:\\proj\\src\\main.rs")), "src/main.rs");
    }
}
