...
pub struct AppState {
    /// Live PTYs, by id.
    /// The registry lock only protects membership. Each terminal has its own
    /// lock so a slow write/resize/snapshot cannot stall unrelated PTYs.
    pub ptys: Mutex<HashMap<String, Arc<Mutex<PtyHandle>>>>,
    /// IDs currently between `openpty` and registry insertion.
    pub spawning_ptys: Mutex<HashSet<String>>,
...
impl AppState {
    pub fn new(db: Connection) -> Self {
        Self {
            ...
            spawning_ptys: Mutex::new(HashSet::new()),
