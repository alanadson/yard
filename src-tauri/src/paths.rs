...
/// Gate every "show this to the user in another program" command goes through.
///
/// `explorer.exe` accepts a path that no longer exists and opens *something*
/// (usually Documentos), so the spawn succeeding says nothing about the click
/// having worked. Asking the disk first is what turns a silent wrong window
/// into a sentence the user can act on.
pub fn must_exist(path: &Path) -> Result<(), String> {
    if path.exists() {
        return Ok(());
    }
    Err(format!(
        "“{}” não está mais no disco (foi movido ou apagado?)",
        path.display()
    ))
}
...
mod tests {
    ...
    /// The regression this locks: "Revelar no Explorer" on a folder that had
    /// been renamed or deleted still spawned `explorer.exe` with a dead path.
    /// The spawn succeeds, so the command answered `Ok` — and Explorer opened
    /// Documentos. Two clicks in the app go through here (`reveal_path`,
    /// `open_external`) and only one of them was checking.
    #[test]
    fn a_vanished_path_is_refused_before_explorer_opens() {
        let alive = std::env::temp_dir();
        assert!(must_exist(&alive).is_ok());

        let dead = alive.join("yard-nao-existe-mesmo-2f9c1");
        let err = must_exist(&dead).unwrap_err();
        assert!(
            err.contains("não está mais no disco"),
            "the message has to explain what happened: {err}"
        );
    }
