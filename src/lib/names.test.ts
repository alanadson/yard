...
describe("uniqueLabels", () => {
  ...
  /**
   * The regression this locks down: the automatic suffix did not look at the
   * names the user typed. With a card literally named "claude (2)" on the
   * board, the second "claude" got that same label — and `yard ask
   * "claude (2)"` delivered the prompt to the first in the list, silently.
   * The address is a contract with agent scripts; two owners cannot exist.
   */
  it("does not repeat a label someone already uses as a name", () => {
    const m = uniqueLabels(
      [item("1", "claude (2)"), item("2", "claude"), item("3", "claude")],
      (i) => i.name,
    );
    expect([...m.values()]).toEqual(["claude (2)", "claude", "claude (3)"]);
    expect(new Set(m.values()).size).toBe(3);
  });

  it("skips as many suffixes as it needs", () => {
    const m = uniqueLabels(
      [
        item("1", "a (2)"),
        item("2", "a (3)"),
