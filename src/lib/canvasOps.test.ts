...
describe("removeNodeAndEdges", () => {
  it("clears rectangle, role, routines and wires of a terminal", () => {
    const c: CanvasData = {
      ...
      roles: { t1: { name: "revisora" }, t2: { name: "outra" } },
      routines: [
    ...
    expect(next.roles).toEqual({ t2: { name: "outra" } });
  ...
  it("drops the roles map entirely when it empties", () => {
    const c: CanvasData = { ...canvas(), roles: { t1: { name: "só essa" } } };
...
describe("placedCorners", () => {
  it("collects the cards and the items that have a corner", () => {
    const c = canvas(
      [
        { id: "n1", type: "note", x: 10, y: 20, w: 100, h: 100, text: "", color: "#fff" },
        { id: "s1", type: "stroke", points: [0, 0, 5, 5], size: "m", color: "#fff" },
        connection("n1", "t1"),
      ],
      { t1: { x: 300, y: 400, w: 640, h: 400 } },
    );
    expect(placedCorners(c)).toEqual([
      { x: 300, y: 400 },
      { x: 10, y: 20 },
    ]);
  });
