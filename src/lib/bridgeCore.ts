...
/**
 * Can the caller already talk to this id?
 *
 * `connect` is the one command that can *widen* an agent's reach, so it needs
 * a rule of its own. Without one, the gate was decorative: an agent that
 * `yard list` showed as isolated could wire itself to every terminal in the
 * group and then talk to all of them — while the manual it reads promises
 * that the user draws the connections.
 *
 * Anchoring each new wire to something the caller already reaches keeps the
 * useful cases (link a note to a teammate, wire two things you were given)
 * and drops the one that made the promise false.
 */
export function reaches(ctx: Ctx, id: string): boolean {
  if (id === ctx.caller.id) return true;
  if (ctx.edges.get(ctx.caller.id)?.has(id)) return true;
  return (
    connectedNotes(ctx).some((n) => n.id === id) ||
    connectedPortals(ctx).some((p) => p.id === id)
  );
}

/** Any entity in the group by name — `connect` resolves both ends with this
 *  and then checks `reaches` on at least one of them. */
 ...
 *
 * Two rules exist because a prompt is arbitrary text and a flag is not:
 *
 * - **`--` ends the flags.** `yard ask "A" -- --raw` sends the literal string
 *   `--raw`. Without it there was no way at all to pass text that happens to
 *   spell a flag: `ask "A" "--raw"` set `raw` and left no prompt, so the CLI
 *   answered with its usage text as if the command were malformed.
 * - **A flag never eats another flag as its value.** `--name --scope global`
 *   used to store `"--scope"` as the name and then lose the scope entirely.
 *   Now the value-taking flag is simply left unset, which every caller
 *   already handles as "not given".
 */
export function parseFlags(args: string[], spec: FlagSpec): ParsedArgs {
  ...
  /** Is the next token a value, or the start of another flag? */
  const value = (i: number): string | undefined => {
    const next = args[i];
    return next === undefined || next === "--" || spec[next] ? undefined : next;
  };
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (kind) {
      ...
      case "stdin":
        ...
        if (a === "--file" && value(i + 1) !== undefined) i++;
        break;
    }
  }
  return out;
...
export function findMentions(text: string, names: string[]): string[] {
  ...
  for (const name of ordered) {
    ...
    for (;;) {
      ...
      const collides = taken.some(([s, e]) => at < e && end > s);
      if (!collides && /\s|[([{,;]/.test(before)) {
        taken.push([at, end]);
        hit.push(name);
        break;
      }
