 ...
 *
 * The catalog (`lib/extensions.ts`) keeps its Portuguese; `t()` is applied
 * here, where the cards are drawn.
 */
...
function Preview({ ext }: { ext: ExtensionDef }) {
  const t = useT();
      ...
      return (
        <div className="ext-preview" aria-hidden="true">
          ...
          <span className="ext-preview-item" style={{ color: "#7ee787" }}>
            {t("→ diagrama desenhado na leitura")}
          </span>
        </div>
      );
    case "katex":
      return (
        <div className="ext-preview" aria-hidden="true">
          ...
          <span className="ext-preview-item" style={{ color: "#d2a8ff" }}>
            {t("→ fórmula desenhada na leitura")}
          </span>
        </div>
      );
    default:
    ...
    <article className={`ext-card ${on ? "is-on" : ""}`}>
      ...
      <div className="ext-info">
        <div className="ext-name-row">
          <span className="ext-name">{t(ext.name)}</span>
          <span className="ext-meta">
            {ext.author} · v{ext.version} · {ext.license}
          </span>
        </div>
        <p className="ext-desc">{t(ext.description)}</p>
        <p className="ext-details">{t(ext.details)}</p>
        <Preview ext={ext} />
      </div>
...
export function ExtensionsModal() {
  const t = useT();
  // `t` is one function for every language; the memo below needs the value
  // that actually changes when the interface flips.
  ...
  const sections = useMemo(() => {
    ...
    // Both languages are searchable: the catalog is Portuguese, the screen
    // may be English, and the user types in whichever they are reading.
    const hit = (ext: ExtensionDef) => {
      if (q === "") return true;
      const label = EXTENSION_KINDS.find((k) => k.id === ext.kind)?.label ?? "";
      const texts = [ext.name, ext.description, ext.details, label];
      return fold(
        `${ext.author} ${texts.join(" ")} ${texts.map((x) => t(x)).join(" ")}`,
      ).includes(q);
    };
  ...
  return (
    <Modal
      ...
      headerExtra={
        <div className="bench-search ext-search">
          ...
          <input
            ...
            placeholder={t("Buscar extensões")}
      ...
      toolbar={
        <div className="ext-scopes" role="group" aria-label={t("Filtrar por categoria")}>
          <button
      ...
      <div className="ext-shelf">
          ...
          <section className="ext-section" key={s.id}>
            <h4 className="ext-section-h">
              ...
              {s.items.some((e) => e.category) && (
                <span className="ext-section-rule">{t("um de cada vez")}</span>
              )}
            </h4>
            {s.items.map((ext) => (
        ...
        {total === 0 && (
          <div className="ext-empty">
            ...
            <p>
              {t("Nenhuma extensão para ")}
              <strong>“{query.trim()}”</strong>
              {kind !== "all" ? t(" nesta categoria") : ""}.
            </p>
