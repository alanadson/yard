...
interface Props {
  ...
  outside?: boolean;
  /**
   * Which grips to mount. Defaults to all eight; a text item passes the four
   * corners, because scaling a line of text is uniform by nature — an edge
   * grip would promise a stretch it cannot deliver.
   */
  dirs?: readonly ResizeDir[];
}

function ResizeHandlesImpl({ onDown, onMove, onUp, outside, dirs }: Props) {
  return (
    <>
      {(dirs ?? RESIZE_DIRS).map((d) => (
        <div
          key={d}
