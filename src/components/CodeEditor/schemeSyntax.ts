...
/**
 * The table itself, so a test can read what paints what. An attribute's value
 * carries `t.attributeValue` and not `t.string` — a tag of its own — so it has
 * to be named here or it renders in the plain text ink (`schemeSyntax.test.ts`).
 */
export function syntaxSpecs(p: SyntaxPalette): TagStyle[] {
  return [
    ...
    { tag: [t.typeName, t.className, t.namespace, t.labelName], color: p.type },
...
function styleOf(p: SyntaxPalette): HighlightStyle {
  return HighlightStyle.define(syntaxSpecs(p));
}
